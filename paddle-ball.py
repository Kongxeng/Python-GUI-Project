...
speed_intvar.set(7) # Initialize y coordinate
...
'''paddle_board = canvas.create_rectangle(250, 500, 350, 520, outline= '#000000', fill='#000FFF')'''                                 
import math
def animate():
...
animate()
player1Up = 0
player1 = canvas.create_rectangle(230, 500, 370, 520, outline= '#000000', fill='#000FFF')

def tick():
    print player1Up
    canvas.move(player1, player1Up, 0)
    canvas.after(1, tick)
tick()
...
root.bind('<KeyPress-Right>', RightPress)
root.bind('<KeyRelease-Right>', RightRelease)
root.bind('<KeyPress-Left>', LeftPress)
root.bind('<KeyRelease-Left>', LeftRelease)
#######
# Event Loop
root.mainloop()